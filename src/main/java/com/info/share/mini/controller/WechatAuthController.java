...
//        response.setStatus(res.getIntValue("code"));
    ...
    @ApiOperation(value = "获取分享二维码", notes = "通过openID, scene参数生成二维码。", httpMethod="POST")
    @PostMapping(value = "/QRImage", produces = {"application/json;charset=UTF-8"})
    public JSONObject getORImage(@RequestBody JSONObject body,
                                 HttpServletResponse response){

        String openId = body.getString("openid");
        if(!userService.checkExists(openId)){
            return JSONObject.parseObject(ResultJSON.success(404, "Sorry, we have none this user.").toSimpleString());
        }
        String scene = body.getString("scene");
        String page = body.getString("page");
        int width = body.getIntValue("width") == 0 ? body.getIntValue("width") : 430;
        JSONObject res = weChatAuthService.getORImage(openId, scene, page, width);
        response.setStatus(res.getIntValue("code"));
        return res;
    }
}
